...
class Locations(Base):
    ...
    def get_location(self, location_id) -> dict or None:  # type: ignore
        for x in self.data:
            if x["id"] == location_id:
                return x
        return None

    def get_locations_in_warehouse(self, warehouse_id) -> list:
        result = []
        for x in self.data:
    ...
    def update_location(self, location_id, location) -> None:
        location["updated_at"] = self.get_timestamp()
