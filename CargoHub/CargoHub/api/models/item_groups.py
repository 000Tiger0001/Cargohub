...
class ItemGroups(Base):
    ...
    def get_item_groups(self) -> list:
        return self.data

    def get_item_group(self, item_group_id) -> dict or None:  # type: ignore
        for x in self.data:
            if x["id"] == item_group_id:
                return x
        return None

    def add_item_group(self, item_group) -> None:
        item_group["created_at"] = self.get_timestamp()
        item_group["updated_at"] = self.get_timestamp()
        self.data.append(item_group)

    def update_item_group(self, item_group_id, item_group) -> None:
        item_group["updated_at"] = self.get_timestamp()
